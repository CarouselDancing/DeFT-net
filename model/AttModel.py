...
class AttModel_fold(Module):
    ...
    def __init__(self, in_features=48, kernel_size=5, d_model=512, num_stage=2, dct_n=10):
        ...
        self.convQ = nn.Sequential(nn.Conv1d(in_channels=in_features * 3, out_channels=d_model, kernel_size=6, bias=False),
                                   nn.ReLU(),
                                   nn.Conv1d(in_channels=d_model, out_channels=d_model, kernel_size=5, bias=False),
                                   nn.ReLU())

        self.convK = nn.Sequential(nn.Conv1d(in_channels=in_features * 3, out_channels=d_model, kernel_size=6, bias=False),
                                   nn.ReLU(),
                                   nn.Conv1d(in_channels=d_model, out_channels=d_model, kernel_size=5, bias=False),
                                   nn.ReLU())
    ...
    def forward(self, src, output_n=25, input_n=50, itera=1):
        ...
        # Adjust the expected input size dynamically based on input channel size
        if src_key_tmp.size(1) == 66:  # If input has 66 channels
            convK_in_features = 66
            convQ_in_features = 66
        else:  # Default to 132 channels
            convK_in_features = 132
            convQ_in_features = 132

        # Update conv layers accordingly
        self.convK[0].in_channels = convK_in_features
        self.convQ[0].in_channels = convQ_in_features
